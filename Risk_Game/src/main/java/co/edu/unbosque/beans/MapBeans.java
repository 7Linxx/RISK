...
public class MapBeans {
	...
	public String init() {
		...
		for (String territory : territories) {
			...
			territorioDao.crear(new TerritoryDTO(territory, jugadorDao.getJugadores().get(selectedPlayer).getColor(),
					r.nextInt(3) + 1));
			jugadorDao.getJugadores().get(selectedPlayer).getTerritoriosPertenecientes().add(territory);
		}
	...
	public void initDetails() {
		...
		textWinner = "";
		enemyPlayer = "";
		currentPlayer = jugadorDao.getPlayers().get(selectedPlayer).getName();
		error1 = error2 = false;
	...
	public String obtainColor(String territory) {
		if (territorioDao.getTerrritories().containsKey(territory)) {
			return territorioDao.getTerrritories().getValue(territory).getColor();
		}
		return "#e6e6e6";
	}
	...
	public void changeTerritory(String territory) {
		if (phase.equals("Phase 1: Reinforce")) {
			if (selectedTerritory != null) {
				territorioDao.getTerrritories().getValue(selectedTerritory.getName())
						.setColor(playerTerritory(selectedTerritory.getName()).getColor());
			}
			if (troopsObtained == 0)
				return;
			if (jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(territory)) {
				selectedTerritory = territorioDao.getTerrritories().getValue(territory);
				territorioDao.getTerrritories().getValue(territory).setColor("#7da028");
				error1 = false;
			} else {
				error1 = true;
			}
			...
			if (jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(territory)
					&& territorioDao.getTerrritories().getValue(territory).getNumberTroops() <= 1) {
				error1 = true;
				offTargetEnemies();
				selectedTerritory = null;
				targetTerritory = null;
			} else if (jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(territory)
					&& territorioDao.getTerrritories().getValue(territory).getNumberTroops() > 1) {
				offTargetEnemies();
				selectedTerritory = territorioDao.getTerrritories().getValue(territory);
				territorioDao.getTerrritories().getValue(territory).setColor("#7da028");
				...
				targetTerritory = territorioDao.getTerrritories().getValue(territory);
			...
			if (selectedTerritory == null && jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(territory)
					&& territorioDao.getTerrritories().getValue(territory).getNumberTroops() <= 1) {
				error2 = true;
				error1 = false;
				targetTerritory = null;
			} else if (selectedTerritory == null
					&& jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(territory)
					&& territorioDao.getTerrritories().getValue(territory).getNumberTroops() > 1) {
				selectedTerritory = territorioDao.getTerrritories().getValue(territory);
				territorioDao.getTerrritories().getValue(territory).setColor("#7da028");
				targetTerritory = null;
				...
				error2 = false;
					...
					&& territorioDao.getTerrritories().getValue(territory).getNumberTroops() > 1) {
				territorioDao.getTerrritories().getValue(selectedTerritory.getName())
						.setColor(jugadorDao.getPlayers().get(selectedPlayer).getColor());
				selectedTerritory = territorioDao.getTerrritories().getValue(territory);
	...
	public void reinforceTerritory() {
		int number = territorioDao.getTerrritories().getValue(selectedTerritory.getName()).getNumberTroops()
				+ troopsCommand;
		territorioDao.getTerrritories().getValue(selectedTerritory.getName()).setNumberTroops(number);
	...
	public PlayerDTO playerTerritory(String territory) {
		for (int i = 0; i < jugadorDao.getPlayers().getSize(); i++) {
			if (jugadorDao.getPlayers().get(i).containsTerritory(territory)) {
				return jugadorDao.getPlayers().get(i);
			}
		}
		return null;
	}
	...
	public void phaseDices() {
		if (phase.equals("Phase 2: Attack")) {
			...
			if (jugadorDao.getPlayers().get(selectedPlayer).getTerritories().getSize() == territories.length) {
				phase = "Winner";
				generateFileResults();
				return;
			}
	...
	public void targetEnemies(String territory) {
		...
		bfs.runSearch();
		for (String aux : territories) {
			if (!jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(aux)) {
				if (bfs.obtainDistance(aux) == 1) {
					territorioDao.getTerrritories().getValue(aux).setColor("#7da028");
	...
	public void offTargetEnemies() {
		...
		for (String aux : enemiesTarget) {
			territorioDao.getTerrritories().getValue(aux).setColor(playerTerritory(aux).getColor());
		}
		enemiesTarget = new MyDoubleLinkedList<>();
	}
	...
	public void battleRound() {
		...
		territorioDao.getTerrritories().getValue(selectedTerritory.getName()).setNumberTroops(remainingATroops);
		territorioDao.getTerrritories().getValue(targetTerritory.getName()).setNumberTroops(remainingBTroops);
		selectedTerritory = territorioDao.getTerrritories().getValue(selectedTerritory.getName());
		targetTerritory = territorioDao.getTerrritories().getValue(targetTerritory.getName());
		if (targetTerritory.getNumberTroops() == 0) {
			textWinner = jugadorDao.getPlayers().get(selectedPlayer).getName() + " lost " + restTroops[0] + " troops | "
					+ playerTerritory(targetTerritory.getName()).getName() + " lost " + restTroops[1] + " troops.";
			textLosser = jugadorDao.getPlayers().get(selectedPlayer).getName() + " has conquered "
			...
			textLosser = jugadorDao.getPlayers().get(selectedPlayer).getName() + " lost " + restTroops[0] + " troops | "
					+ playerTerritory(targetTerritory.getName()).getName() + " lost " + restTroops[1] + " troops";
			winner = 2;
		} else {
			textWinner = jugadorDao.getPlayers().get(selectedPlayer).getName() + " lost " + restTroops[0] + " troops.";
	...
	public void addAfterConquist(String territory) {
		...
		jugadorDao.getPlayers().get(selectedPlayer).getTerritories().add(territory);
	}

	public void addTroopsAfterConquist() {
		int resTroops = selectedTerritory.getNumberTroops() - troopsCommand;
		territorioDao.getTerrritories().getValue(selectedTerritory.getName()).setNumberTroops(resTroops);
		territorioDao.getTerrritories().getValue(targetTerritory.getName()).setNumberTroops(troopsCommand);
		phase = "Phase 2: Attack";
		offTargetEnemies();
		selectedTerritory = null;
		targetTerritory = null;
			...
			public void runSearch() {
				...
				while (!queue.isEmpty()) {
					...
					for (int v : getGraph().getAdj().get(u)) {
						if (getDis()[v] == -1 && jugadorDao.getPlayers().get(selectedPlayer)
		...
		for (String aux : territories) {
			if (jugadorDao.getPlayers().get(selectedPlayer).containsTerritory(aux)) {
				if (bfs.obtainDistance(aux) != -1 && !aux.equals(territory)) {
					territorioDao.getTerrritories().getValue(aux).setColor("#ccd9c1");
	...
	public void strengthenTerritory() {
		...
		territorioDao.getTerrritories().getValue(selectedTerritory.getName()).setNumberTroops(restTroops);
		int receivedTroops = targetTerritory.getNumberTroops() + troopsCommand;
		territorioDao.getTerrritories().getValue(targetTerritory.getName()).setNumberTroops(receivedTroops);
		selectedTerritory = territorioDao.getTerrritories().getValue(selectedTerritory.getName());
		targetTerritory = territorioDao.getTerrritories().getValue(targetTerritory.getName());
		offTargetAllies();
		reforzed = true;
	...
	public void passTurn() {
		selectedPlayer++;
		if (selectedPlayer == jugadorDao.getPlayers().getSize()) {
			selectedPlayer = 0;
		}
		while (jugadorDao.getPlayers().get(selectedPlayer).getTerritories().getSize() == 0) {
			selectedPlayer++;
			if (selectedPlayer == jugadorDao.getPlayers().getSize()) {
				selectedPlayer = 0;
			}
	...
	public void receiveTroops() {
		troopsObtained = jugadorDao.getPlayers().get(selectedPlayer).getTerritories().getSize() / 3;
		if (troopsObtained < 3) {
			troopsObtained = 3;
		}
	}
	...
	public boolean initPlayers() {
		...
		for (int i = 0; i < numPlayers; i++) {
			...
			jugadorDao.getPlayers().add(new PlayerDTO(names[i], passwords[i], colors[i], emails[i]));
		}
		return required;
	}
	...
	public void saveGame() {
		...
		for (TerritoryDTO current : territorioDao.getTerrritories().values()) {
		...
		int i = 0;
		for (PlayerDTO current : jugadorDao.getPlayers()) {
	...
	public String loadGame() {
		...
		territorioDao = new TerritoryDAO();
		jugadorDao = new PlayerDAO();
		...
		for (String object : jsTerritories) {
			...
			territorioDao.create(territory);
		}
		...
		for (int i = 0; i < jsPlayers.length; i++) {
			jugadorDao.create(pPlayers.getValue(i));
		}
	...
	public void generateFileResults() {
		...
		int totalTroops = 0;
		for (String aux : territories)
			totalTroops += territorioDao.getTerrritories().getValue(aux).getNumberTroops();
		String content = "Thank you very much for playing the game. Congratulations "
				+ jugadorDao.getPlayers().get(selectedPlayer).getName() + " you won the game with a total of "
				+ totalTroops + " troops. Thanks also to the other players you played very well.";
		String[] lplayers = new String[jugadorDao.getPlayers().getSize() + 1];
		lplayers[0] = "Players List:";
		for (int i = 0; i < jugadorDao.getPlayers().getSize(); i++) {
			lplayers[i + 1] = jugadorDao.getPlayers().get(i).getName() + ". "
					+ (i == selectedPlayer ? "Winner." : "Loser");
		}
	...
	public String endGame() {
		for (int i = 0; i < jugadorDao.getPlayers().getSize(); i++) {
			PlayerDTO current = jugadorDao.getPlayers().get(i);
	...
	public void sendEmailToPlayers(String name, String email, boolean isWinner) {
		...
		for (int i = 0; i < jugadorDao.getPlayers().getSize(); i++) {
			content += jugadorDao.getPlayers().get(i).getName() + ". "
					+ (i == selectedPlayer ? "Winner.\n" : "Loser\n");
		}
		try {
	...
	public String register() {
		...
		for (PlayerDTO p : jugadorDao.getPlayers()) {
